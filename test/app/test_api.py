...
@pytest.mark.parametrize(
    ...
    [
        ...
        (TEST_JSON_no_time, WEEK_BEFORE, TODAY, True),
    ],
)
def test_index_image(mocked_client, request_body, expected_start_date, expected_end_date, expected_saving):
    mocked_client.app.state.imagery_store = TestImageryStore()

    response = mocked_client.post('/raster', json=request_body)
...
def test_errorneous_json(mocked_client):
    response = mocked_client.post('/raster', json={})
...
def test_imagery_store_raises_assertion_error_should_fail(mocked_imagery_store):
    ...
    response = app.post('/raster', json=TEST_JSON_start_end)
